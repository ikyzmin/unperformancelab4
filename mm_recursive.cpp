...
template <typename T>
void mm_internal (T *C, const T *A, const T *B, int n, int length)
/* Effect: Compute
 *    C+=A*B,
 ...
 */
{
    ...
    else
    {
        ...
        T const *A12 = A         + mid;
        T const *A21 = A + n*mid      ;
        T const *A22 = A + n*mid + mid;

        T const *B11 = B              ;
        T const *B12 = B         + mid;
        T const *B21 = B + n*mid      ;
        T const *B22 = B + n*mid + mid;
...
template <typename T>
void mm_recursive_parallel(T *C, const T *A, const T *B, int n)
             ...
             const char* test_name, bool verify = true, bool timer = true)
{
    ...
    if (timer)
    {
        ...
        __cilkview_report(&start_data,&end_data, test_name, CV_REPORT_WRITE_TO_RESULTS);
